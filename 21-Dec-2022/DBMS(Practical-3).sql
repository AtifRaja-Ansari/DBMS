...
# min
select dept,min(Esalary) as 'min_dpt_salary' from Future_Employee group by dept;
select min(Esalary) from Future_Employee;
#subquery min
select Ename,dept,Esalary from future_employee where Esalary=(select max(Esalary) from Future_Emp);
# max
